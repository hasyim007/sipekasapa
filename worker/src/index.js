 ...
 *   POST { action: 'saveConfig', instansi, sekolah, alamat, kepsekNama, kepsekNip, logoBase64 }
 * ============================================================
 */
...
async function getConfig(env) {
    const row = await env.DB.prepare(
        'SELECT instansi, sekolah, alamat, kepsekNama, kepsekNip, logoBase64 FROM config WHERE id = 1'
    ).first();
    return row || { instansi: '', sekolah: '', alamat: '', kepsekNama: '', kepsekNip: '', logoBase64: '' };
}

async function saveConfig(env, data) {
    const logo = data.logoBase64 || '';
    if (logo.length > 300000) {
        return { success: false, message: 'Ukuran logo terlalu besar. Gunakan logo yang lebih kecil (maks. ~30KB file asli).' };
    }
    await env.DB.prepare(
        `INSERT INTO config (id, instansi, sekolah, alamat, kepsekNama, kepsekNip, logoBase64)
         VALUES (1, ?, ?, ?, ?, ?, ?)
